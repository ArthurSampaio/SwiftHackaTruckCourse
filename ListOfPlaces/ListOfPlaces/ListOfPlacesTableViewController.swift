...
class ListOfPlacesTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        self.places = PlaceDAO.getPlaces()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "placeIdentifier", for: indexPath)

        //conversion of types
        if let placeCell = cell as? PlaceTableViewCell{
            
            let place = self.places[indexPath.row]
            placeCell.nameOfPlace.text = place.name
            placeCell.pictureOfPlace.image = UIImage(named: place.picture)
